...
export async function getGmiConnectionStatus(): Promise<GmiConnectionStatus> {
  ...
  const { data } = await admin
    ...
    .select("base_url, is_active, last_verified_at")
    .eq("id", true)
    .maybeSingle();
  return {
    configured: !!data,
...
/** Speichert/aktualisiert den API-Key (und optional die Base-URL). */
...
export async function verifyGmiConnection(): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    await verifyConnection({ baseUrl: data.base_url, apiKey: data.api_key });
    await admin
      .from("getmyinvoices_connection")
